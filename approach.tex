...
Second, it evaluates whether the rewritten SQL improves execution efficiency using available database feedback, such as estimated optimizer cost and execution latency.
%
...
For simple questions over clear schemas, it chooses a lightweight workflow that activates only basic evidence construction and a single SQL generation strategy.
...
When problems are detected, the controller can revise the current plan by requesting additional evidence construction, activating alternative generation strategies, invoking repair, retrieving more relevant optimization cases, or switching to another rewrite direction.
...
It records selected strategies, schema and value evidence, generated SQL candidates, execution errors, repair attempts, optimization hints, retrieved cases, rewritten SQLs, validation results, and cost changes.
The Meta-Cognitive Controller uses this memory to avoid repeating failed actions and to provide context for subsequent reflection, retry, and termination decisions.
...
It contains expert rules and successful historical rewrite cases that have been validated by the Validator Agent.
A rewrite is added to the AEKB only when it satisfies two conditions: the rewritten SQL is semantically consistent with the original SQL, and it improves execution cost.
Each accepted case is summarized as a rewrite template record:
\[
\langle T_{\mathit{src}}, T_{\mathit{dst}}, C, D \rangle .
\]
Here, \(T_{\mathit{src}}\) denotes the source SQL template, \(T_{\mathit{dst}}\) denotes the rewritten SQL template, \(C\) specifies the applicability constraints, and \(D\) describes the optimization rationale.
%
...
The normalized template is then compared with the \texttt{srcSQL} templates in the AEKB using a hybrid similarity score that combines sequence similarity and token-set overlap.
The top-ranked records are returned as candidate rewrite knowledge for the Explain Analyzer Agent and SQL Rewriter Agent.
...
\begin{itemize}[leftmargin=10pt]
	...
	These tools help the system ground natural language expressions into relevant tables, columns, values, and join paths.
	...
	The execution plan analyzer normalizes raw execution plans into plan-level risk signals, while the SQL quality analyzer detects static SQL risks from the parsed AST.
	The semantic consistency checker compares the original and rewritten SQLs on the available database state, and the cost evaluator measures estimated optimizer cost, execution latency before and after rewriting.
	...
	These tools allow the Explain Analyzer Agent and SQL Rewriter Agent to reuse prior optimization experience and update the knowledge base after successful rewrites.
\end{itemize}

Together, these tools allow \toolname\ to ground reasoning in database evidence, analyze execution behavior, retrieve prior optimization experience, and update memory after successful rewrites.
