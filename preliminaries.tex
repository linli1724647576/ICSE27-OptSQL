...
Existing Text-to-SQL systems are typically designed as static pipelines that decompose query generation into pre-defined stages, including schema linking, candidate generation, and post-hoc execution validation.
...
Inspired by enactive cognition and meta-cognitive theory, we reformulate Text-to-SQL as a dynamic interaction process between an intelligent system and the database environment.
...
The Meta-Cognitive Controller observes signals such as schema linking ambiguity, candidate disagreement across generated SQLs, and convergence behavior in optimization loops.
For instance, if multiple candidate SQLs consistently disagree on join paths, the controller may trigger re-linking of schema entities or restart generation with alternative evidence.
...
The system treats execution results, query plans, and cost estimations as fundamental feedback signals that continuously shape its optimization behavior.
When execution traces reveal inefficiencies such as full table scans, suboptimal join orders, or redundant computation paths, the system  invokes an optimization agent that reasons over both the current execution feedback and previously observed optimization experiences to produce improved SQL variants.
Crucially, optimization is not performed in isolation for each query. Instead, the system continuously accumulates experience from execution outcomes and rewriting actions, forming an evolving Auto-Evaluation Knowledge Base (AEKB). This knowledge base stores historical rewrite cases, execution feedback, and observed performance patterns, enabling the agent to generalize from past optimization experiences to new queries.
In this way, query optimization is transformed from a rule-based or purely local rewriting process into an experience-driven learning loop, where execution feedback serves as the foundation for progressively improving future reasoning and optimization decisions.
